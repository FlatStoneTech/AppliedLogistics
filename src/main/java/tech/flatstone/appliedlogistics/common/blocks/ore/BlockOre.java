...
public class BlockOre extends BlockBase implements IBlockRenderer {
    ...
    public void registerBlockRenderer() {
        for (int i = 0; i < EnumOres.values().length; i++) {
            if (!EnumOres.byMeta(i).isVanillaGen()) {
                ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(this), i, new ModelResourceLocation(ModInfo.MOD_ID + ":" + "ore_" + EnumOres.byMeta(i).getName(), "inventory"));
            } else {
                ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(this), i, new ModelResourceLocation(EnumOres.byMeta(i).getName() + "_ore", "inventory"));
            }
        }
    }
}
